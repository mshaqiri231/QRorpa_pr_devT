@extends('layouts.appMasterDelivery')
@section('content')
...
?>
@include('inc.menuDeliveryComp.menuDeliveryStyle')
<script>
var last = "";
var lastV = 0;
...
@if ($openStat2D[2] == '0')
    ...
    <script>
        // PER Pishininen (31,32,33) - bllokohet perdorimi i platformes
...
<div class="ProdModalBody">
    ...
    <div class="col-md-3 left-section">
        @include('inc.menuDeliveryComp.leftSection')
    </div>
    <div class="col-md-3 right-section">
        @include('inc.menuDeliveryComp.rightSection')    
    </div>
    <!-- Recomendet Products -->
    <div class="recommended-mobile">@include('inc.menuDeliveryComp.menuDeliveryRecomendet')</div>
    <div class="col-md-6 center-section">
        
        <div class="recommended-desktop">@include('inc.menuDeliveryComp.menuDeliveryRecomendet')</div>
        <!-- Show Menu -->
        @include('inc.menuDeliveryComp.menuDeliveryShowMenu')
    </div>

    @include('inc.menuDeliveryComp.menuDelScript')
        ...
        if(searchWord != ""){
            ...
            $.ajax({
                ...
                success: function(res){
                    ...
                    $('#produktetUl').append('<h4 class="text-center">'+Object.keys(res).length+' '+$('#products_found').val()+'  </h4>');
                    // var count = 0;
                                                ...
                                                '<div class="col-lg-6 col-sm-12 col-md-12 product-section">'+
                                                    '<div class="row">'+
                                                        ...
                                                        '<div class="col-2 add-plus-section">'+
                                                            '<button class="btn mt-2 noBorder" type="button"><i class="fa fa-plus fa-2x" style="color:#27beaf"></i></button>'+
                                                        '</div>'+
                                                    '</div>'+
                                                '</div>'+
                                                '<div class="col-lg-3 col-sm-0 col-md-0"></div>'+
...
function addThis2(theId, prodId, name, value, prodQ, nameClick) {
    ...
    if (checkBox.checked == true) {
        ...
        var prices = document.getElementsByClassName("price" + prodId);
        for (var i = 0; i < prices.length; i++) {
            var newV = parseFloat(prices.item(i).innerText) * value;
        ...
        if (ExtraCartV != '') {
            ...
            for (var i = 0; i < extras.length; i++) {
                ...
                if (lastV != 0) {
                    var newQ = (extras2D[1] / lastV) * value;
                    var newQ = newQ.toFixed(2);
                } else {
                    var newQ = extras2D[1] * value;
                    var newQ = newQ.toFixed(2);
                }
        ...
        var extrasToSaveTwo = TotPrice.value - QmimiBazeValue;

        TotPrice.value = ((QmimiBazeValue * value) + extrasToSaveTwo).toFixed(2);
...
function addThisRec(theId, prodId, name, price) {

    if (lastVRec != 0) {
        price = price * lastVRec;
        price = price.toFixed(2);
    }
...
function addThis2Rec(theId, prodId, name, value, prodQ) {
    var checkBox = document.getElementById(theId);
    ...
    if (checkBox.checked == true) {
        ...
        if (ExtraCartV != '') {
            ...
            for (var i = 0; i < extras.length; i++) {
                ...
                if (lastVRec != 0) {
                    var newQ = (extras2D[1] / lastVRec) * value;
                    var newQ = newQ.toFixed(2);
                } else {
                    var newQ = extras2D[1] * value;
                    var newQ = newQ.toFixed(2);
...
<div class="ProdModalBody">
    ...
    @foreach(DeliveryProd::where('toRes',$_GET["Res"])->where('accessableByClients','1')->get() as $prod)
            ...
            <div class="modal-content" style="border-radius:30px;">
                <div class="modal-body">
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col-10">
                                <h4 class="modal-title">{{$prod->emri}} <span style="color:rgb(72,81,87);">
                                                ...
                                                <div class="container-fluid mt-2 firstTwoTypes{{$prod->id}}">
                                                    <div class="row ml-1" style="margin-bottom:-15px;">
                                                        ...
                                                        <div class="col-9 d-flex" style="margin-left:-35px;" onclick="addThis2('llojetPE{{$thisType->id}}O{{$prod->id}}','{{$prod->id}}','{{$thisType->emri}}','{{$vleraType}}','{{$starterPrice}}','True')">
                                                            <p style="width:70%;" class="text-left color-text"><strong>{{$thisType->emri}}</strong></p>
                                ...
                                <?php
                                    $countNewTypes = 1;
                                       if(count(explode('--0--', $prod->type)) > 0){
                                                ...
                                                if($countNewTypes++ > 5){
                                                    if(!empty($llP)){
                                                        ...
                                                        if(!empty($thisType)){
                                                            ...
                                                            echo '
                                                                <div class="container AllTypesToHide IDType'.$prod->id.'">
                                                                    <div class="row ml-1">
                                            ...
                                            echo '<p class="text-center opacity-65">'.__("inc.empty").'</p>';
                                        }
                                    ?>
                            </div>
                                                    ...
                                                    <div class="container ">
                                                        <div class="row ml-1">
                                                            ...
                                                            <div class="col-9 text-left d-flex" style="margin-left:-35px;" onclick="addThis('extPE{{$thisExtra->id}}O{{$prod->id}}','{{$prod->id}}','{{$thisExtra->emri}}','{{$thisExtra->qmimi}}','True')">
                                                                <p style="width:70%;" class="text-left color-text"><strong>{{$thisExtra->emri}}</strong></p>
                                            ...
                                            if($countNewEtx++ > 5){
                                                if(!empty($extP)){
                                                    $thisExtra = ekstra::find(explode('||',$extP)[0]);
                                                        if($thisExtra != null){
                                                        $theIdOfThisEkstra = "extP'.$thisExtra->id.'O'.$prod->id.'" ;
                                                        echo '
                                                        <div class="container AllExtrasToHide IDExtra'.$prod->id.'">
                                                            <div class="row ml-1">
                                                                ...
                                                                <div class="col-9 d-flex" style="margin-left:-35px;" bis_skin_checked="1" onclick="addThis(\''.$theIdOfThisEkstra.'\',\''.$prod->id.'\',\''.$thisExtra->emri.'\',\''.$thisExtra->qmimi.'\',\'True\')">
                                                                    <p style="width:70%;" class="text-left"><strong>'.$thisExtra->emri.'</strong> </p> <p style="width:30%;" class="text-right"><span class="price'.$prod->id.'"> '.sprintf('%01.2f', $thisExtra->qmimi).'</span> <sup>'.__("inc.currencyShow").'</sup>  
                                                            </p> 
                                                                </div>
...
<div class="modal" id="priceChangeMenuAlert" style="margin-top:70%;">
  <div class="modal-dialog">
    <div class="modal-content" style="border-radius:20px;">
      ...
      <!-- Modal body -->
      <div class="modal-body text-center" >
       <strong id="priceChangeMenuAlertMB">{{__('inc.productPriceChange10Min')}}</strong> 
...
<script>
                    ...
                    function timeReset(){
                        ...
                        if((time >= '20:00:00' && time <= '20:00:05') || (time >= '23:59:55' && time <= '00:00:01') ){
                            ...
                            $('#priceChangeMenuAlertMB').html($('#priceChange20m').val());
                        }else if(time >= '19:50:00' && time <= '19:50:05'){
                            $('#priceChangeMenuAlert').modal('show');
                            $('#priceChangeMenuAlertMB').html($('#priceChange10m').val());
                        }else if(time >= '19:55:00' && time <= '19:55:05'){
                            $('#priceChangeMenuAlert').modal('show');
                            $('#priceChangeMenuAlertMB').html($('#priceChange5m').val());
                        }else if(time >= '19:59:00' && time <= '19:59:05'){
                            $('#priceChangeMenuAlert').modal('show');
                            $('#priceChangeMenuAlertMB').html($('#priceChange1m').val());
                        }
                        // console.log(time);
                    }
                    ...
                    $(document).ready(function () {
                        ...
                        $.ajax({
                            url: '{{ route("adsModuleSa.getAdsForMenuDelivery") }}',
                            method: 'post',
                            dataType: 'json',
                            data: {
                            ...
                            success: (response) => {
                                if(response != 'empty'){
                                    ...
                                    setInterval(function () {
                                        $.ajax({
                                            url: '{{ route("adsModuleSa.getAdsForMenuDeliveryRepeatable") }}',
                                            method: 'post',
                                            dataType: 'json',
                                            data: {
                                                resI: $('#thisRestaurant').val(),
