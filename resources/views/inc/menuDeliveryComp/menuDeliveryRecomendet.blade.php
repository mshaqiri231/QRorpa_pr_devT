    ...
    <div class="col-lg-6 col-sm-12 teksti">
        ...
        <div class="swiper-container col-lg-6 p-0" style="border-top-left-radius:50px; border-bottom-left-radius:50px; overflow: hidden; ">
            <div class="swiper-wrapper" id="recProdList">
                @foreach(DeliveryProd::where([['toRes', '=', $theRes],['recomendet','=', '1'],['accessableByClients','1']])->orderBy('recomendetNr')->get() as $RePro)
                    @if(kategori::findOrFail($RePro->kategoria)->acsByCldDE == 1)
                        <div class="swiper-slide recProElement" data-toggle="modal" data-target="#Prod{{$RePro->id}}" data-backdrop="static" data-keyboard="false">
                            <img style="width:120px; height:120px; border-radius:50%;" src="storage/DeliveryRecomendet/{{$RePro->recomendetPic}}"
                                alt="image">
                            <p style=" width:100%; font-size:13px;" class="color-text"><strong>
                                    {{$RePro->emri}}
                                </strong></p>
