<?php
    ...
    $totDelBr = number_format(0, 2, '.', ' ');
    $totDelMwst = number_format(0, 2, '.', ' ');
    $totDelNe = number_format(0, 2, '.', ' ');
...
?>
    ...
    @if ($showTable)
                ...
                <tr>
                    <td colspan="6"><strong>Lieferung (Bar)</strong></td>
                </tr>
                    ...
                    @if ($orOne->payM == "Barzahlungen" || $orOne->payM == "Cash")
                        ...
                        @foreach (explode('---8---',$orOne->porosia) as $porOne)
                            <?php 
                                ...
                                if($theDP != NULL){ $mwstPRC = number_format($theDP->mwstForPro, 2, '.', ' '); }else{ $mwstPRC = 2.60; }
                ...
                @endforeach
                <?php
                    $totDelSasia += number_format($sumSasia, 0, '.', ' ');
                ?>
                    <tr class="heading">
                        <td style="text-align: left;">
                ...
                @if ( $totDiscount > 0)
                    ...
                    <?php
                        ...
                        $totDelBr += number_format($brutoAfterD, 2, '.', ' ');
                        $totDelMwst += number_format($mwstAfterD, 2, '.', ' ');
                        $totDelNe += number_format($netoAfterD, 2, '.', ' ');
                    ?>
                    <tr class="heading">
                ...
                @else
                    <?php
                        $totDelBr += number_format($sumBruto, 2, '.', ' ');
                        $totDelMwst += number_format($sumMwst, 2, '.', ' ');
                        $totDelNe += number_format($sumNeto, 2, '.', ' ');
                    ?>
                @endif
    @endif
    ...
    @if ($showTable)
                ...
                <tr>
                    <td colspan="6"><strong>Lieferung (Karte)</strong></td>
                </tr>
    ...
    <tr class="heading">
        <td colspan="2" style="text-align: left; background: #6e6e6e !important; color:white;">
            <strong> Summen für Lieferung </strong>
        </td>
        <td style="text-align: right; background: #6e6e6e !important; color:white;">
            <strong>CHF {{number_format($totDelBr, 2, '.', ' ')}}</strong>
        </td>
        <td style="text-align: right; background: #6e6e6e !important; color:white;">
            <strong>CHF {{number_format($totDelMwst, 2, '.', ' ')}}</strong>
        </td>
        <td style="text-align: right; background: #6e6e6e !important; color:white;">
            <strong>CHF {{number_format($totDelNe, 2, '.', ' ')}}</strong>
        </td>
        <td style="text-align: right; background: #6e6e6e !important; color:white;">
            <strong>{{number_format($totDelSasia, 0, '.', ' ')}} X</strong>
        </td>
    </tr>
    @endif
