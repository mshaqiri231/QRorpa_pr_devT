...
<section class="pr-2 pl-2 pb-4">
    <button class="btn btn-block p-2 backQr" style="border-radius:20px;" data-toggle="modal" data-target="#addStWorker">
    ...
    <div class="d-flex flex-wrap justify-content-between mt-3 mr-4 ml-4">
        ...
        @foreach(StatusWorker::where('toRes', $thisRestaurantId)->get()->sortByDesc('created_at') as $sWor)
            ...
            <div style="width:30%" class="text-center">
                <button style="width:90%" class="delSW{{$sWor->id}} btn btn-block btn-outline-info" data-toggle="modal" data-target="#editStWorker{{$sWor->id}}"><strong>{{__('adminP.toEdit')}}</strong></button>
            </div>
            <div style="width:30%" class="text-center">
    ...
    <!-- The Modal -->
    <div class="modal" id="addStWorker" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="false" 
    ...
    @foreach(StatusWorker::where('toRes', $thisRestaurantId)->get()->sortByDesc('created_at') as $sWor)
        ...
        <!-- The Modal -->
        <div class="modal" id="editStWorker{{$sWor->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="false" 
        style="background-color: rgba(0, 0, 0, 0.5); padding-top:5%;">
            <div class="modal-dialog">
                <div class="modal-content">
