...
<link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.css">
                        ...
                        <?php
                            $OrderAllCount = OrdersPassive::where([['Restaurant',$thisRestaurantId],['statusi','!=','2'],['servedBy',Auth::User()->id]])
                            ->whereMonth('created_at', $getFromM)->whereYear('created_at', $getFromY)->whereIn('nrTable',$myTablesWaiter)->orderByDesc('created_at')->get()->count();

                            if(isset($_GET['orSk']) && $_GET['orSk'] > 0){
                                $showOr = OrdersPassive::where([['Restaurant',$thisRestaurantId],['statusi','!=','2'],['servedBy',Auth::User()->id]])
                                ->whereMonth('created_at', $getFromM)->whereYear('created_at', $getFromY)->whereIn('nrTable',$myTablesWaiter)->orderByDesc('created_at')->skip($_GET['orSk'])->take(1000)->get();
                                $showGCSales = Null;
                            }else{
                                $showOr = OrdersPassive::where([['Restaurant',$thisRestaurantId],['statusi','!=','2'],['servedBy',Auth::User()->id]])
                                ->whereMonth('created_at', $getFromM)->whereYear('created_at', $getFromY)->whereIn('nrTable',$myTablesWaiter)->orderByDesc('created_at')->take(1000)->get();
                                $showGCSales = giftCard::where([['toRes',$thisRestaurantId],['soldByStaff',Auth::user()->id],['oldGCtransfer','0']])->whereMonth('created_at', $getFromM)->whereYear('created_at', $getFromY)->get();
                            }                            
                            // ->skip(500)
                        ?>
    ...
    <div class="pl-3 pr-3 pt-3 pb-3 d-flex flex-wrap justify-content-between" id="salesTypeStats">
        ...
        @foreach ($showOr as $orCash)
            @if ($orCash->payM == 'Barzahlungen' || $orCash->payM == 'Cash')
                ...
                ?>
            @elseif ($orCash->payM == 'Kartenzahlung')
                <?php 
                    if ($orCash->inCashDiscount > 0){
        ...
        @foreach (giftCard::where([['soldByStaff',Auth::user()->id],['oldGCtransfer','0']])->whereMonth('created_at', $getFromM)->whereYear('created_at', $getFromY)->get() as $gcPMOne)
                                ...
                                if(($monthCount >= $resCreatedM && $yearCount == $resCreatedY) || $yearCount > $resCreatedY ){
                                    echo "
                                        <div class='b-qrorpa color-white text-center p-2 m-2' style='border-radius:20px; width:25%; font-size:19px; font-weight:bold;'>
                                            <a class='color-white anchorHover' href='admWoMngStatistics02Waiter?mo=".$monthCount."&ye=".$yearCount."'>
                                                ( ".$monthCount." )";
                                                switch($monthCount){
...
<div class="d-flex justify-content-between">
    ...
    @if (!isset($_GET['orSk']) || (isset($_GET['orSk']) && $_GET['orSk'] == 0))
        <button class="btn btn-dark mb-2" style="width:10%;"><strong>0-{{$firstMax}}</strong></button>
    @else
        @if(isset($_GET['mo']) && isset($_GET['ye']))
        <?php $mo=$_GET['mo']; $ye=$_GET['ye']; ?>
        <button onclick="location.href = 'admWoMngStatistics02Waiter?orSk=0&mo={{$mo}}&ye={{$ye}}';" class="btn btn-outline-dark mb-2" style="width:10%;"><strong>0-{{$firstMax}}</strong></button>
        @else
        <button onclick="location.href = 'admWoMngStatistics02Waiter?orSk=0';" class="btn btn-outline-dark mb-2" style="width:10%;"><strong>0-{{$firstMax}}</strong></button>
        @endif
    @endif
    ...
    @foreach (range(1, 8) as $nrRend) 
        ...
        @if ($OrderAllCount > $rngSt && $OrderAllCount >= $rngEd)
            ...
            @else
                @if(isset($_GET['mo']) && isset($_GET['ye']))
                <?php $mo=$_GET['mo']; $ye=$_GET['ye']; ?>
                <button onclick="location.href = 'admWoMngStatistics02Waiter?orSk={{$rngSt}}&mo={{$mo}}&ye={{$ye}}';" class="btn btn-outline-dark shadow-none mb-2" style="width:10%;"><strong>{{$rngSt+1}}-{{$rngEd}}</strong></button>
                @else
                <button onclick="location.href = 'admWoMngStatistics02Waiter?orSk={{$rngSt}}';" class="btn btn-outline-dark shadow-none mb-2" style="width:10%;"><strong>{{$rngSt+1}}-{{$rngEd}}</strong></button>
                @endif
            @endif
        @elseif ($OrderAllCount > $rngSt)
            ...
            @else
                @if(isset($_GET['mo']) && isset($_GET['ye']))
                <?php $mo=$_GET['mo']; $ye=$_GET['ye']; ?>
                <button onclick="location.href = 'admWoMngStatistics02Waiter?orSk={{$rngSt}}&mo={{$mo}}&ye={{$ye}}';" class="btn btn-outline-dark shadow-none mb-2" style="width:10%;"><strong>{{$rngSt+1}}-{{$OrderAllCount}}</strong></button>
                @else
                <button onclick="location.href = 'admWoMngStatistics02Waiter?orSk={{$rngSt}}';" class="btn btn-outline-dark shadow-none mb-2" style="width:10%;"><strong>{{$rngSt+1}}-{{$OrderAllCount}}</strong></button>
                @endif
            @endif
        @else
                    ...
                    <tr class="openOrderRow">
                        ...
                        <td>    
                            ...
                            @elseif($order->inPercentageDiscount > 0)
                                <?php
                                    $shumaBef = number_format($order->shuma - $order->tipPer, 4, '.', '');
                                    $theDi = number_format($shumaBef * ($order->inPercentageDiscount * 0.01), 4, '.', '');
                                ?>
