...
<section class="pl-4 pr-4 pb-5">
    ...
    <div class="d-flex justify-content-between">
        ...
        @include('adminPanelWaiter.taTempProd.dashboardTakeawayNewOrd')
    ...
    @if(Orders::where([['Restaurant', Auth::user()->sFor],['nrTable',500],['servedBy',Auth::user()->id]])->whereBetween('created_at', [$today_str, $today_end])->limit(2)->count() >0)
        ...
        @if(Orders::where([['Restaurant', Auth::user()->sFor],['nrTable',500],['statusi', '<',2],['servedBy',Auth::user()->id]])->whereBetween('created_at', [$today_str, $today_end])->limit(2)->count() > 0)
        <table class="table table-hover" id="desktopTable">
            <thead>
            <tr>
                <th style="opacity:50%;">{{__('adminP.time')}}</th>
                <th style="opacity:50%;">{{__('adminP.products')}}</th>
                <th style="opacity:50%;">{{__('adminP.total')}}/Code</th>
                <th class="text-center" style="opacity:50%;"></th>
            ...
            <tbody>
                @foreach(Orders::where([['Restaurant', Auth::user()->sFor],['nrTable',500],['statusi', '<',2],['servedBy',Auth::user()->id]])->whereBetween('created_at', [$today_str, $today_end])
                        ...
                        <tr class="openOrderRow" id="taOrderRow{{$order->id}}">
                            ...
                            <td style="width:200px;" class="text-center d-flex flex-wrap justify-content-between">
                                ...
                                <div style="width:100%; display: none;" id="cancelCommentDiv{{$order->id}}" class="form-group mb-2 mt-2">
                                    ...
                                    <button onclick="sendCancelRequest('{{$order->id}}')" style="margin:0px;" class="mb-1 btn-block btn btn-dark shadow-none" type="button" id="sendCancelRequestBtn">
    ...
    @foreach(Orders::where([['Restaurant', Auth::user()->sFor],['nrTable',500],['statusi','<=', 1],['servedBy',Auth::user()->id]])->whereBetween('created_at', [$today_str, $today_end])
            ->get() as $order)
        <?php
...
<script>
    ...
    function sendCancelRequest(orId){
        if(!$('#cancelCommentInp'+orId).val()){
            ...
            $('#sendCancelRequestBtn').html('<img style="width: 20%; margin-left:40%; margin-right:40%;" src="storage/gifs/loading2.gif" alt="">');
            $('#sendCancelRequestBtn').prop('disabled', true);
            $.ajax({
