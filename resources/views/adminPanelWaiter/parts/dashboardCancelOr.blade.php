...
<div class="p-1 pb-4">
    ...
    <button data-toggle="modal" data-target="#otherMonthsStatsMonth" class="btn btn-block otherMonthsBtn" id="tabletTableBtn" style="display:none;"> {{__('adminP.previousMonths')}} </button>
        ...
        <tbody>
            @foreach(OrdersPassive::where([['Restaurant',$thisRestaurantId],['statusi','2']])->whereMonth('created_at', $getFromM)->whereYear('created_at', $getFromY)->whereIn('nrTable',$myTablesWaiter)
...
<script>
    ...
    @foreach(OrdersPassive::where([['Restaurant',$thisRestaurantId],['statusi','2']])->whereMonth('created_at', $getFromM)->whereYear('created_at', $getFromY)->whereIn('nrTable',$myTablesWaiter)->get() as $order)
        <?php
