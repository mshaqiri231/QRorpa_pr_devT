...
use App\OPSaferpayReference;
    ...
    // Auth::User()->id
    $waOrdsForTd = OrdersPassive::where([['Restaurant',$thisRestaurantId],['statusi','!=','2'],['servedBy',Auth::User()->id]])->whereIn('nrTable',$myTablesWaiter)
    ->whereDate('created_at', '=', Carbon::today())->orderByDesc('created_at')->get();
...
<div class="pl-3 pr-3 pt-3 pb-3 d-flex flex-wrap justify-content-between" id="salesTypeStats">
    ...
    @foreach (giftCard::where([['soldByStaff',Auth::user()->id],['oldGCtransfer','0']])->whereDate('created_at', '=', Carbon::today())->get() as $gcPMOne)
...
<div class="p-1 pb-4">
    ...
    <table class="table table-hover" id="desktopTable" style="display:none;">
        ...
        <tbody>
            @foreach (giftCard::where([['toRes',Auth::user()->sFor],['soldByStaff',Auth::user()->id],['oldGCtransfer','0']])->whereDate('created_at', Carbon::today())->get() as $gcSaleOne)
                <tr class="openOrderRow">
                    <td>
                        <?php
...
<script>
    ...
    @foreach(OrdersPassive::where([['Restaurant',$thisRestaurantId],['statusi','!=','2'],['servedBy',Auth::User()->id]])->whereDate('created_at', Carbon::today())->whereIn('nrTable',$myTablesWaiter)->get() as $order)
        <?php
