                                ...
                                @endif
                                    ...
                                    @foreach(Takeaway::where('toRes', '=', Auth::user()->sFor)->where('kategoria','=',$taCatsOneId)->orderByDesc('created_at')->get() as $ketoProd)
                                        <div class="d-flex justify-content-between" style="width:24.4%; margin: 4px 0.05% 4px 0.05%; border:1px solid rgb(39,190,175); border-radius:0.25rem;">
                                            ...
                                            <button onclick="addToCartExpressTATemp('{{$ketoProd->id}}','{{$sndNotiActv}}','{{$ketoProd->emri}}','{{$ketoProd->qmimi}}')" id="addToCartExpressBtn{{$ketoProd->id}}" class="btn btn-outline-dark shadow-none" style="width:20%; height:100%; margin:0px; padding:1px;">
                                            @else
...
@include('adminPanelWaiter.taTempProd.payMethodsScript')
@include('adminPanelWaiter.taTempProd.payCashModal')
@include('adminPanelWaiter.taTempProd.payRechnungModal')
...
<script>
    ...
    function addToCartExpressTATemp(taProId,notiAct, taProEmri, taProQmimi){
        ...
        ordSndExp++;
        var showNewOrdFast ='<button class="btn btn-outline-danger mb-1" style="margin:0px; padding:1px; width:12%;" onclick="deleteTempTAProd(0)" id="deleteTempTAProdBtn0" disabled>'+
                                '<img style="width:32.4px; height:auto;" src="storage/gifs/loading2.gif">'+
                            '</button>'+
                            '<p class="mb-1 productShowTATemp" style="margin:0px; padding:1px; width:62%; overflow:hidden; white-space: nowrap;">'+
                                '<span>1 X</span>'+
                                '<strong> '+taProEmri+'</strong>'+
                            '</p>'+
                            '<p class="mb-1 text-center" style="margin:0px; padding:1px; width:25%;">'+
                                '<strong>'+parseFloat(taProQmimi).toFixed(2)+' <span style="font-size: 0.4rem;;">CHF</span></strong>'+
                            '</p>';
        $('#addOrForTAModalBody2RightPayments').append(showNewOrdFast);
        $.ajax({
            url: '{{ route("dash.waiterCheckForCheckin") }}',
            method: 'post',
            data: {_token: '{{csrf_token()}}'},
            success: (checkCHINRespo) => {
                if($.trim(checkCHINRespo) == 'CheckInValide'){
                    ...
                    $.ajax({
                        ...
                        success: () => {
                            $("#addOrForTAModalBody2Right").load(location.href+" #addOrForTAModalBody2Right>*", function( response, status, xhr ) {
                                ordDonExp++;
                                if(parseInt(ordSndExp) == parseInt(ordDonExp)){
