    ...
    .teksti{
        ...
        .teksti i{
            margin-right:20px
                ...
                <div id="deliverySortingAll">
                    <a href="{{route('admWoMng.adminWoDeliveryWaiter')}}" style="width:100%;" class="btn btn-dark"><strong>{{__('adminP.delivery')}}</strong></a>
                    ...
                    @foreach(kategori::where('toRes',  Auth::user()->sFor)->get()->sortBy('positionDelivery') as $kat)
                        @if(DeliveryProd::where('toRes', Auth::user()->sFor)->where('kategoria', $kat->id)->get()->count() > 0)
                            <div style="cursor: pointer; position:relative; object-fit: cover;" class="col12 p-1 text-center">
                                ...
                                @if(strlen($kat->emri) > 20)
                                    <div class="teksti d-flex" style="font-size:20px;  margin-bottom:13px;">          
                                        ...
                                        <p data-toggle="modal" data-target="#changeDeliveryCatPositionTel{{$kat->id}}" class="pr-3"><i class="fas fa-sort"></i>{{$kat->positionDelivery}}</p>
                                    </div>
                                @else
                                ...
                                <div class="modal fade" id="changeDeliveryCatPositionTel{{$kat->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true"
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-body d-flex flex-wrap justify-content-between">
                                                ...
                                                <button style="width:49%" class="btn btn-success" onclick="changeDeliveryCatPositionSaveTel('{{$kat->id}}')">{{__('adminP.saveOnComputer')}}</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <ul class="ProdLists" id="prodsOfDeTel{{$kat->id}}" style="display:none;">
                                @foreach(DeliveryProd::where('kategoria', $kat->id)->get()->sortBy('position') as $allDe)
                                    <li style="background-color:rgb(189, 246, 240)" class="d-flex">
                                        <p class="text-center" style="width: 12.5%;" data-toggle="modal" data-target="#changeDeliveryProdPositionTel{{$allDe->id}}"><strong><i class="fas fa-sort"></i></strong></p>
                                        <p class="text-center" style="width: 12.5%;"><strong>{{$allDe->position}} #</strong></p>
                                        <p style="width: 75%;"><strong>{{$allDe->emri}}</strong></p>
                                    </li>

                                    <!-- Change Delivery Category Position Modal -->
                                    <div class="modal fade" id="changeDeliveryProdPositionTel{{$allDe->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true"
                                        data-backdrop="false"style="background-color: rgba(0, 0, 0, 0.5); padding-top:5%;">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-body d-flex flex-wrap justify-content-between">
                                                    <h3 class="pt-3" style="width: 85%;"><strong>{{$allDe->emri}}</strong></h3>
                                                    <button style="width: 15%; font-size:23px;" type="button" class="btn btn-default text-right" data-dismiss="modal"><strong>X</strong></button>

                                                    <hr style="width: 100%;">
                                                    <p style="width:49%; font-size:18px;" class="text-right pt-2"><strong>{{__('adminP.position')}} : </strong></p>        
                                                    <input class="form-control text-center mb-2" type="number" style="width:49%; border:none; border-bottom: 1px solid black; font-size:22px; font-weight:bold;" 
                                                        value="{{$allDe->position}}" id="changeDeliveryProdPositionVal{{$allDe->id}}">
                                                    
                                                    <button style="width:49%" class="btn btn-danger" data-dismiss="modal">{{__('adminP.abort')}}</button>
                                                    <button style="width:49%" class="btn btn-success" onclick="changeDeliveryProdPositionSaveTel('{{$allDe->id}}','{{$kat->id}}')">{{__('adminP.saveOnComputer')}}</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    function showProKatTel(kId){
        // $('.ProdLists').hide();
        if($('#prodsOfDeTel'+kId).is(":visible")){
            $('#prodsOfDeTel'+kId).hide();
        }else{
            $('#prodsOfDeTel'+kId).show();
        }
    }
    ...
    function changeDeliveryProdPositionSaveTel(prodId,catId){
        $.ajax({
            ...
            success: () => {
                $('#changeDeliveryProdPositionTel'+prodId).modal('toggle')
                $("#prodsOfDeTel"+catId).load(location.href+" #prodsOfDeTel"+catId+">*","");
            },
            error: (error) => {
                console.log(error);
                alert($('apleaseUpdateAndTryAgain').val());
            }
        });
    }
    ...
    function changeDeliveryCatPositionSaveTel(catId){
        $.ajax({
            ...
            success: () => {
                $('#changeDeliveryCatPositionTel'+catId).modal('toggle')
                $("#deliverySortingAll").load(location.href+" #deliverySortingAll>*","");
            },
            error: (error) => {
                console.log(error);
