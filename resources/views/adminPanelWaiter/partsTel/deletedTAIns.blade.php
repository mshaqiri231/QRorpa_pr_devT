...
use Illuminate\Support\Facades\Auth;
    ...
    if(Auth::user()->id == 443 || Auth::user()->id == 442){
        ...
        if(isset($_GET['mo']) && isset($_GET['ye'])){
            $taOrDel = tabOrderDelete::where([['toRes',Auth::user()->sFor],['byId',Auth::user()->id]])
...
<link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.css">
    ...
    <div class="d-flex justify-content-between mb-2">
        @if(isset($_GET['mo']) && isset($_GET['ye'])){
            <p style="color:rgb(72,81,87); width:25%; font-weight:bold; font-size:1.3rem; margin:0;" class="text-center">{{sprintf("%02d", $_GET['mo'])}}.{{$_GET['ye']}}</p>
        @else
            <p style="color:rgb(72,81,87); width:25%; font-weight:bold; font-size:1.3rem; margin:0;" class="text-center">{{sprintf("%02d", Carbon::now()->month)}}.{{Carbon::now()->year}}</p>
        @endif
                        ...
                        <?php
                            ...
                            while(true){
                                if(($monthCount >= $resCreatedM && $yearCount == $resCreatedY) || $yearCount > $resCreatedY ){
                                    echo "
                                        <div class='color-white text-center p-2 mb-1' style='border-radius:7px; width:100%; font-size:19px; font-weight:bold; background-color:rgb(72,81,87);'>
                                            <a class='color-white anchorHover' href='WaiterstatsDeletedTAProdsPage?mo=".$monthCount."&ye=".$yearCount."'>
                                                ( ".$monthCount." )";
                                                switch($monthCount){
