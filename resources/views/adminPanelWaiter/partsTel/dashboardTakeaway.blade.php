...
<section class="pl-1 pr-1 pb-5">
    <div class="d-flex flex-wrap justify-content-between">
        ...
        <a class="btn shadow-none" href="{{route('admWoMng.indexAdmMngPageWaiter',['tabs'])}}" style="width:49%; margin:0px; border:1px solid rgb(39,190,175); color:rgb(39,190,175);">
        ...
        @include('adminPanelWaiter.taTempProdTel.dashboardTakeawayNewOrd')
                ...
                ->get()->sortByDesc('created_at') as $order)
                    ...
                    <tr id="taOrderRow{{$order->id}}">
                        ...
                        <td colspan="1" class="text-center" data-toggle="modal" data-target="#openOrderTel{{$order->id}}Tel">
                            ...
                            <p>{{number_format($order->shuma - $order->inCashDiscount, 2, ',','')}}<sup>{{__('adminP.currencyShow')}}</sup></p>
                            <p style="font-size:0.9rem; margin:0px; padding:0px;">( Rabat:{{number_format($order->inCashDiscount - $order->dicsountGcAmnt, 2, ',','')}}<sup>{{__('adminP.currencyShow')}}</sup>)</p>
