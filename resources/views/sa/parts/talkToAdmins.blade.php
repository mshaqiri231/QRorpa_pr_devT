...
<div class="p-3 pb-5">
    ...
    <div class="d-flex justify-content-between mt-2 mb-2">
        <h3 style="color:rgb(39,190,175); width:49%;"><strong>Sprich mit Restaurants</strong></h3>
                ...
                <div class="card-body p-1">
                    <div class="d-flex flex-wrap">
                        ...
                        <?php
                            ...
                            $us = User::findOrFail($oneAv->avForId);
                        ?>
                        <h5 style="width:100%; color:white; margin-top:-5px;" class="card-title d-flex text-center">
                            @if($us != NULL)
                            <span style="width: 33%;">Zum: {{$us->name}}</span>
                            @else
                            <span style="width: 33%;">Zum: NULL</span>
                            @endif
                            <span style="width: 33%;">{{$koha2D[0]}}:{{$koha2D[1]}}</span> 
                            <span style="width: 33%;">{{$data2D[2]}}/{{$data2D[1]}}/{{$data2D[0]}}</span>
                ...
                <div class="card-body p-1">
                    <div class="d-flex flex-wrap">
                        ...
                        <?php
                            $data2D = explode('-',explode(' ',$oneAv->created_at)[0]);
                            $koha2D = explode(':',explode(' ',$oneAv->created_at)[1]);
                            $us = User::findOrFail($oneAv->avById);
                        ?>
                        <h5 style="width:100%; color:white; margin-top:-5px;" class="card-title d-flex text-center">
                            @if($us != NULL)
                            <span style="width: 33%;">Aus: {{$us->name}}</span>
                            @else
                            <span style="width: 33%;">Aus: NULL</span>
                            @endif
                            <span style="width: 33%;">{{$koha2D[0]}}:{{$koha2D[1]}}</span> 
                            <span style="width: 33%;">{{$data2D[2]}}/{{$data2D[1]}}/{{$data2D[0]}}</span>
                        </h5>
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="p-1 d-flex flex-wrap justify-content-start" id="addNewConvCardAdmins">
                    <p style="width: 100%; color:rgb(39,190,175); font-size:1.8rem; margin:0px;"><strong>Admins</strong></p>
                    @foreach (User::where('role',5)->get() as $oneAdm)
                        <div class="mb-1 unselectedAdm text-center" id="selectAddNewConvCardAdmBtn{{$oneAdm->id}}" onclick="selectAddNewConvCardAdm('{{$oneAdm->id}}')">
                            <span style="margin-top:-4px;">{{$oneAdm->name}}</span>
                            <?php $tr = Restorant::find($oneAdm->sFor);?>
                            @if($tr != NULL)
                            <br><span style="color:rgb(72,81,87); margin-top:-4px;"><strong>{{$tr->emri}}</strong></span>
                            @else
                            <br><span style="color:rgb(72,81,87); margin-top:-4px;"><strong>NULL</strong></span>
                            @endif
                        </div>
                    @endforeach
                </div>
                <hr>
                <div class="p-1 d-flex flex-wrap justify-content-start" id="addNewConvCardWaiters">
                    <p style="width: 100%; color:rgb(39,190,175); font-size:1.8rem; margin:0px;"><strong>Waiters</strong></p>
                    @foreach (User::where('role',55)->get() as $oneAdm)
                        <div class="mb-1 unselectedAdm text-center" id="selectAddNewConvCardAdmBtn{{$oneAdm->id}}" onclick="selectAddNewConvCardAdm('{{$oneAdm->id}}')">
                            <span style="margin-top:-4px;">{{$oneAdm->name}}</span>
                            <?php $tr = Restorant::find($oneAdm->sFor);?>
                            @if($tr != NULL)
                ...
                <input type="hidden" id="addNewConvCardAdmin" value="0">
                ...
                <div class="alert alert-danger text-center mt-1 mb-1" style="display:none;" id="addNewConvCardError02">Wählen Sie zuerst einen Administrator aus!</div>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function selectAddNewConvCardAdm(admId){
        if($('#addNewConvCardAdmin').val() != 0){
            $('#selectAddNewConvCardAdmBtn'+$('#addNewConvCardAdmin').val()).removeClass('selectedAdm');
            $('#selectAddNewConvCardAdmBtn'+$('#addNewConvCardAdmin').val()).addClass('unselectedAdm');
        }
        $('#selectAddNewConvCardAdmBtn'+admId).removeClass('unselectedAdm');
        $('#selectAddNewConvCardAdmBtn'+admId).addClass('selectedAdm');
        $('#addNewConvCardAdmin').val(admId);
    }

    function saveNewConvCard(sadmID){
        if(!$('#addNewConvCardResonInput').val()){
            if($('#addNewConvCardError01').is(':hidden')){ $('#addNewConvCardError01').show(50).delay(4000).hide(50); }
        }else if($('#addNewConvCardAdmin').val() == 0){
            if($('#addNewConvCardError02').is(':hidden')){ $('#addNewConvCardError02').show(50).delay(4000).hide(50); }
        }else{
            $.ajax({
				url: '{{ route("atsMsg.SAASaveNewCard") }}',
				method: 'post',
				data: {
					superadminId: sadmID,
                    adminSel: $('#addNewConvCardAdmin').val(),
    ...
    function sendMsg2(avId, sadmId){
        if(!$('#messageInputFor'+avId).val()){
            if($('#messagesDivError01').is(':hidden')){ $('#messagesDivError01').show(50).delay(4000).hide(50); }
        }else{
            $.ajax({
				url: '{{ route("atsMsg.SAASaveNewMessageOnCard2") }}',
				method: 'post',
				data: {
					superadminId: sadmId,
                    avId: avId,
