  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h4 class="modal-title">Shto nje kuzhinier</h4>
      ...
      <div class="modal-body">
        ...
        {{Form::open(['action' => 'UserAdminControler@storeKuzhinier', 'method' => 'post']) }}
            ...
            <div class="form-group">
                ...
                <select name="user" class="form-control ">
                    ...
                    <?php
                        if(!empty(User::all())){
                            foreach(User::where('role', '!=', 3)->get()->sortByDesc('created_at') as $usr){
...
<div class="text-center">
    <h3 class="color-qrorpa"><strong>Kuzhinier</strong></h3>
    <hr>
</div>
<div class="container">
    <div class="row">
        ...
        <div class="col-6">
            <button class="btn btn-block btn-outline-dark noBorder" data-toggle="modal" data-target="#addKuzhinier">
                <strong>+ Shto nje kuzhinier</strong>
            </button>
        </div>
        <div class="col-5">
            <!-- Actual search box -->
            <div class="form-group has-search">
                ...
                <input type="text" id="searchKuzhinier" class="form-control noBorder" placeholder="Search Users">
            </div>
        </div>
    </div>
</div>

    <table id="userKuzhinier" class="table table-striped table-bordered" style="width:100%">
        <thead>
            <tr>
                <th>ID</th>
                <th>Name</th>
        ...
        <tbody>
            @foreach(User::where('role', '=', 3)->get()->sortByDesc('created_at') as $kuzh)
                <tr>
                    <td>{{$kuzh->id}}</td>
                    <td>{{$kuzh->name}}</td>
                    <td>{{$kuzh->email}}</td>
                    ...
                    <td class="text-center">
                        <button onclick="deleteKuzh('{{$kuzh->id}}')" class="btn btn-block btn-outline-danger">
                            Delete
                        </button>
                    </td>
                </tr>
...
<script>
      $(document).ready(function() {
            ...
            $("#searchKuzhinier").on("keyup", function() {
                var value = $(this).val().toLowerCase();
                $("#userKuzhinier tr").filter(function() {
        ...
        function deleteKuzh(tid){
            ...
            $.ajax({
                ...
                success: (response) => {
                    $('#userKuzhinier').load('/userKuzhinier #userKuzhinier', function() {});
                },
                error: (error) => {
                    console.log(error);
