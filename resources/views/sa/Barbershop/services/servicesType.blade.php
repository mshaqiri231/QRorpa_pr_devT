...
<!-- Add Type Modal -->
<div class="modal" id="addType" role="dialog" aria-labelledby="myModalLabel" data-backdrop="false" 
  ...
  <div class="modal-dialog modal-md">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h4 class="modal-title"> Add a Type </h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>

    
            {{Form::open(['action' => 'BarbershopTypeController@store', 'method' => 'post', 'enctype' => 'multipart/form-data' ]) }}
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        {{ Form::label('Value', null, ['class' => 'control-label color-black']) }}
                        {{ Form::number('value','', ['class' => 'form-control ', 'min' => '0', 'step'=> '0.000000000001', 'required',]) }}
                    </div>
                </div>
...
<section class="pl-4 pr-4 pt-4 pb-5">
    <div class="d-flex justify-content-between">
        ...
        <h3 style="width:45%;" class="text-right color-qrorpa pr-4 pt-4"><strong>Type</strong></h3>
    </div>

    <button class="btn btn-block addTypeBtn mt-3 mb-3" data-toggle="modal" data-target="#addType"> Add a new Type </button>
    ...
    <div class="d-flex justify-content-between flex-wrap">
        @foreach(BarbershopCategory::where('toBar', $barbershopId)->get()->sortByDesc('created_at') as $bCat)
            <div style="width:32%;" class="catTypeBlock p-3 d-flex flex-wrap justify-content-between" id="catDiv{{$bCat->id}}">
                ...
                <br>
                <p style="width:20%" class="text-right pr-3"><strong>Value</strong></p>
                ...
                @foreach(BarbershopType::where([['toBar', $barbershopId],['kategoria', $bCat->id]])->get()->sortByDesc('created_at') as $type)
                    <p style="width:20%" class="text-right pr-3">{{$type->vlera}}</p>
                    <p style="border:1px solid lightgray; width:1%;"></p>
                    <p style="width:64%"  class="pl-2">{{$type->emri}}</p>
                    <button style="width:5%;" class="btn btn-outline default" data-toggle="modal" data-target="#editType{{$type->id}}">
                        <i class="far fa-xl fa-edit"></i>
                    </button>
                    <button onclick="deleteBT('{{$type->id}}','{{$bCat->id}}')" style="width:5%;" class="btn btn-outline default">
                        ...
                        <!-- Edit Type Modal -->
                        <div class="modal" id="editType{{$type->id}}" role="dialog" aria-labelledby="myModalLabel" data-backdrop="false" 
                            style="background-color: rgba(0, 0, 0, 0.5); margin-top:5%;">
                        <div class="modal-dialog modal-md">
                            <div class="modal-content">
                            ...
                            <div class="modal-header">
                                ...
                                <button type="button" class="close" data-dismiss="modal">&times;</button>
                                    ...
                                    {{Form::open(['action' => 'BarbershopTypeController@update', 'method' => 'post']) }}
                                        ...
                                        <div class="modal-body">
                                            <div class="form-group">
                                                {{ Form::label('Name', null, ['class' => 'control-label color-black']) }}
                                                {{ Form::text('emri', $type->emri, ['class' => 'form-control ', 'required']) }}
                                            </div>
                                            ...
                                            <div class="form-group">
                                                {{ Form::label('Value', null, ['class' => 'control-label color-black']) }}
                                                {{ Form::number('value', $type->vlera, ['class' => 'form-control ', 'min' => '0', 'step'=> '0.000000000001', 'required',]) }}
                                            </div>
                                        </div>

                                        {{ Form::hidden('barbershop', $barbershopId , ['class' => 'form-control ']) }}
                                        {{ Form::hidden('typeId', $type->id , ['class' => 'form-control ']) }}
...
<script>
    function deleteBT(tId,bId){
        $.ajax({
						url: '{{ route("barType.BTDelete") }}',
						method: 'post',
						data: {
							id: tId,
							_token: '{{csrf_token()}}'
