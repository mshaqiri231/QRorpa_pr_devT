<?php
    ...
    if(isset($_GET['mo']) && isset($_GET['ye']) && isset($_GET['Res'])){
        $theRes = $_GET['Res'];
        $theMonth = $_GET['mo'];
        $theYear = $_GET['ye'];
    }else{
    ...
    $pPlus = PiketLog::where('toRes', $theRes)->where('piket', '>' , '0')->get()->sum('piket');
    $pMinus = PiketLog::where('toRes', $theRes)->where('piket', '<' , '0')->get()->sum('piket') * (-1);
...
<div class="modal" id="otherMonths" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="false" 
  ...
  <div class="modal-dialog modal-xl">
    ...
    <div class="d-flex justify-content-between flex-wrap mt-2" style="font-size:19px;">
        ...
        @foreach(PiketLog::where('toRes', $theRes)->whereMonth('created_at', $theMonth)->whereYear('created_at',$theYear)->get()->sortByDesc('created_at') as $resPo)
