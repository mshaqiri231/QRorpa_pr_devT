...
<section class="ml-2 mr-2 mt-2 p-3 b-white" style="border-radius:35px;" id="profileDesktop">
    <div class="text-center">
        <h3 class="color-qrorpa"><strong>{{__('profile.welcome')}} {{explode(' ',Auth::user()->name)[0]}}</strong></h3>
    ...
    <div class="mt-2 d-flex justify-content-between">
        ...
        @if(Auth::user()->profilePic == 'empty')
            <img style="width:10%; height:200px;"  src="/storage/images/ProfileIcon.png" alt="img" data-toggle="modal" data-target="#profilePicAdd">
        @else
            <img style="width:10%; height:200px;"  src="/storage/profilePics/{{Auth::user()->profilePic}}" alt="img" data-toggle="modal" data-target="#profilePicAdd">
        @endif

        <div style="width:30%; height:fit-content;" id="profileDesktopLeft">
            <h2 style="font-weight:bold; font-size:35px;">{{Auth::user()->name}}</h2> 
            ...
            <div class="input-group mb-1">
                ...
                <input type="text" class="form-control shadow-none" style="font-weight: bold;" id="userEmailInput" onkeyup="showProfileSaveEmail()" value="{{Auth::user()->email}}">
            ...
            <div class="input-group mb-1" style="display: none;" id="saveNewEmailConfCodeDiv">
                ...
                <input type="text" class="form-control shadow-none" id="userEmailInputCode" placeholder="{{__('profile.confirmation_code')}} xxxxxx">
            ...
            <div class="input-group">
                ...
                <input style="font-weight: bold;" type="text" id="saveNewPhNr01Input" class="form-control shadow-none" onkeyup="showProfileSavePhoneNr()" value="{{Auth::user()->phoneNr}}">
            ...
            <div class="input-group mb-1" style="display: none;" id="saveNewPNumberConfCodeDiv">
                ...
                <input type="text" class="form-control shadow-none" id="userPNumberInputCode" placeholder="{{__('profile.confirmation_code')}} xxxxxx">
                <div class="input-group-append">
            ...
            <div class="input-group">
                ...
                placeholder="Schreiben Sie Ihre Telefonnummer" aria-label="Schreiben Sie Ihre Telefonnummer (Optional)">
                <div class="input-group-append" style="display: none;" id="profileSavePhoneNr">
            ...
            <p class="clickPointer" id="changePasswordP" onclick="showChangePass()" style="font-size: 16px; color:red; opacity:0.75;"><strong><i class="fas fa-key"></i> {{__('profile.changeThePassword')}}</strong></p>
            <div id="changePasswordDiv" style="display: none;">
                <div class="input-group mb-1">
                    ...
                    <input type="password" class="form-control shadow-none" id="currentPass" name="currentPass">
                    <div class="input-group-append">
                ...
                <div class="input-group mb-1">
                    ...
                    <input type="password" class="form-control shadow-none" id="newPass" name="newPass">
                    <div class="input-group-append">
        ...
        <div style="width:55%; height:fit-content;" class="d-flex justify-content-between flex-wrap">
            <div style="width:100%;" class="d-flex flex-wrap justify-content-between">

                <div style="width: 32%; padding:10px; border-radius:6px; margin-bottom:10px;" class="jumbotron jumbotron-fluid">
                    <div class="container">
                ...
                <div style="width: 32%; padding:10px; border-radius:6px; margin-bottom:10px;" class="jumbotron jumbotron-fluid">
                    <div class="container">
                        <pre style="margin-bottom: 3px; font-weight:bold;">{{__('profile.payMethodCash')}}    : {{number_format((float)$cashPaySum, 2, '.', '')}} <sup>{{__('profile.currency')}}</sup></pre>
                        <pre style="margin-bottom: 3px; font-weight:bold;">{{__('profile.payMethodOnline')}} : {{number_format((float)$onlinePaySum, 2, '.', '')}} <sup>{{__('profile.currency')}}</sup></pre>
                        <pre style="margin-bottom: 3px; font-weight:bold;">{{__('profile.payMethodAll')}} : {{number_format((float)$cashPaySum+$onlinePaySum, 2, '.', '')}} <sup>{{__('profile.currency')}}</sup></pre>
                    </div>
                </div>
                <div style="width: 32%; padding:10px; border-radius:6px; margin-bottom:10px;" class="jumbotron jumbotron-fluid">
                    <div class="container">
            ...
            <h3 style="width:100%; font-weight:bold; color:rgb(72,81,87);"> {{__('profile.urOrdersOnQRorpa')}} </h3>
           
            <p style="width:10%; font-weight:bold; color:rgb(72,81,87);"> <i class="fas fa-2x fa-list-ol"></i></p>
            <p style="width:25%; font-weight:bold; color:rgb(72,81,87);">{{__('profile.restaurant')}}</p>
            <p class="text-center" style="width:15%; font-weight:bold; color:rgb(72,81,87);">{{__('profile.table')}}</p>
            <p class="text-center" style="width:20%; font-weight:bold; color:rgb(72,81,87);">{{__('profile.total')}}</p>
            <p style="width:15%; font-weight:bold; color:rgb(72,81,87);"><i class="fas fa-2x fa-money-bill-wave"></i></p>
            <p style="width:10%; font-weight:bold; color:rgb(72,81,87);"><i class="far fa-2x fa-file-pdf"></i></p>
            <!-- <p style="width:5%;"></p> -->
            <div style="width:100%">
                <hr style="margin:-5px 0px 5px 0px; ">
            </div>
            @foreach(Orders::where('byId',Auth::user()->id)->orderByDesc('created_at')->get() as $orOne)
                <p style="width:10%; font-weight:bolder; color:rgb(72,81,87);">{{$orOne->refId}}</p>
                @if(Restorant::find($orOne->Restaurant) != NULL)
                <p style="width:25%; font-weight:bolder; color:rgb(72,81,87);">{{Restorant::find($orOne->Restaurant)->emri}}</p>
                @else
                <p style="width:25%; font-weight:bolder; color:red;">NULL</p>
                @endif
                <p class="text-center" style="width:15%; font-weight:bolder; color:rgb(72,81,87);">
                ...
                <p class="text-center" style="width:20%; font-weight:bolder; color:rgb(72,81,87);">{{$orOne->shuma}} <sup>{{__('profile.currency')}}</sup></p>
                <p style="width:15%; font-weight:bolder; color:rgb(72,81,87);">
                @if($orOne->payM == 'Cash')
                ...
                <form style="width:10%; font-weight:bold; color:rgb(72,81,87);" method="POST" action="{{ route('receipt.getReceipt') }}">
                    {{ csrf_field()}}
    ...
    <div class="modal" id="profilePicAdd">
        <div class="modal-dialog">
            ...
            <div class="modal-footer d-flex justify-content-between">
                <button style="width:45%;" type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
            ...
            $.ajax({
                ...
                success: (respo) => {
                    ...
                    if(respo == 'codeFail'){
                        ...
                        $("#profileDesktopLeft").load(location.href+" #profileDesktopLeft>*","");
                    }
                },
                error: (error) => { console.log(error); }
            });
