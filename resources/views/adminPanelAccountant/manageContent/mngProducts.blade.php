...
<div class="container-fluid mb-4 " >
    <div class="row mt-4">
        ...
        <div class="col-8 text-center">
            <p style="font-size:45px;" class="color-qrorpa">"{{Restorant::find($theResId)->emri}}"  /  {{__('adminP.products')}}</p>
        </div>
         <div class="col-2"></div>
    </div>
    ...
    <div class="row mb-3">
        <div class="col-12">
            <button type="button" class="btn btn-outline-dark btn-block" data-toggle="modal" data-target="#addProduktModal">
                <img src="https://img.icons8.com/dusk/64/000000/add-property.png" width="25"> <strong>{{__('adminP.addNewProduct')}}</strong></button>
        </div>
    </div>
</div>
    ...
    @foreach(Produktet::where('toRes',$theResId)->get()->sortByDesc('created_at') as $prod)
            ...
            <div style="width:49.7%;" class="d-flex justify-content-around mt-2 border p-1">
                ...
                <div style="width:43%;">
                    <h5 class="color-black">{{sprintf('%01.2f', $prod->qmimi) }}{{__('adminP.currencyShow')}}
                            @if($prod->qmimi2 != 999999)
                                ( {{sprintf('%01.2f', $prod->qmimi2) }} {{__('adminP.currencyShow')}})
                            @endif
                    </h5>
                ...
                <div style="width:15%;" class="mt-2">
                    <button class="btn btn-outline-info btn-block mb-1" data-toggle="modal" data-target="#editProdM{{$prod->id}}">{{__('adminP.toEdit')}}</button>
                         {{Form::open(['action' => ['ProduktController@destroy', $prod->id ], 'method' => 'POST', 'onsubmit' => 'return confirm("__('adminP.areYouSure')")'])}}
    ...
    style="background-color: rgba(0, 0, 0, 0.5); margin-top:5%;">
            <div class="modal-dialog modal-lg pb-5" >
                ...
                <div class="modal-header">
                    <h4 class="modal-title">{{__('adminP.addNewProductForRestaurant')}}</h4>
                            ...
                            <div class="col-2">
                                <div class="form-check mt-4">
                                    <label class="form-check-label">
                                        <input type="radio" class="form-check-input" name="tipiK" value="6">{{__('adminP.kitchen')}}
                                    </label>
                                </div>
                                <div class="form-check mt-5">
                                    <label class="form-check-label">
                                        <input type="radio" class="form-check-input" name="tipiK" value="7">{{__('adminP.bar')}}
                                    </label>
                                </div>
                            </div>
                        </div>
