...
@foreach(kategori::where('toRes',Auth::user()->sFor)->get(); as $kategorit)
    ...
    <div class="modal  fade " id="editThisCategory{{$kategorit->id}}Tel" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="false" 
        ...
        <div class="modal-dialog modal-lg">
            ...
            {{Form::open(['action' => ['KategoriController@updateAdminP', $kategorit->id], 'method' => 'post' , 'enctype' => 'multipart/form-data']) }}
                ...
                <div class="modal-body">
                    ...
                    <div id="addCatModalPhotoSel{{$kategorit->id}}Tel">
                        <div id="selPic{{$kategorit->id}}Phase01Tel">
                            <div class="custom-file mb-3 mt-3">
                                {{ Form::label(__('adminP.image'), null , ['class' => 'custom-file-label']) }}
                                {{ Form::file('foto', ['class' => 'custom-file-input', 'id'=> 'customFileCategoryTel'.$kategorit->id]) }}
                            </div>
                            <button type="button" data-toggle="modal" data-target="#otherPicture{{$kategorit->id}}Tel"
                        ...
                        <div class="d-flex justify-content-between mt-3 mb-3" style="display:none !important;" id="selPic{{$kategorit->id}}Phase02Tel">
                            <p style="width:10%;"></p>
                            <p style="width:30%; font-weight:bold;" class="text-center" id="selPic{{$kategorit->id}}Phase02NameTel"></p>
                            <p style="width:30%; font-weight:bold;" class="text-center" id="selPic{{$kategorit->id}}Phase02TitleTel"></p>
                            <button type="button" style="width:20%;" class="btn btn-outline-danger" onclick="resetAddCatModalPIcLibTel('{{$kategorit->id}}')">{{__('adminP.resetToDefault')}}</button>
                            <p style="width:10%;"></p>
                        </div>
                        ...
                        <div class="row mt-3 mb-2" >
                            <div class="col-12 form-check" style="margin-left: 15px;">
                                ...
                                <label class="form-check-label pl-2" for="catAccessByClientsEditRes{{$kategorit->id}}"><strong>Kundenzugang zur Kategorie (Restaurant)</strong></label>
                        ...
                        <div class="row mt-1 mb-2" >
                            <div class="col-12 form-check" style="margin-left: 15px;">
                                ...
                                <label class="form-check-label pl-2" for="catAccessByClientsEditTA{{$kategorit->id}}"><strong>Kundenzugang zur Kategorie (Takeaway)</strong></label>
                        ...
                        <div class="row mt-1 mb-2" >
                            <div class="col-12 form-check" style="margin-left: 15px;">
                                ...
                                <label class="form-check-label pl-2" for="catAccessByClientsEditDE{{$kategorit->id}}"><strong>Kundenzugang zur Kategorie (Lieferung)</strong></label>
                                <input type="hidden" id="catAccessByClientsValEditDE{{$kategorit->id}}" name="catAccessByClientsValEditDE" value="1">
                            </div>
                        </div>
                        
                        {{ Form::hidden('photoFrom',1, ['class' => 'form-control' , 'id' => 'photoFromID'.$kategorit->id.'Tel']) }}
                        {{ Form::hidden('photo','', ['class' => 'form-control', 'id' => 'photoID'.$kategorit->id.'Tel']) }}

                        {{ Form::hidden('isAccountant','1', ['class' => 'form-control']) }}
                    </div>
                </div>
    ...
    <div class="modal" id="otherPicture{{$kategorit->id}}Tel" role="dialog" aria-labelledby="myModalLabel" data-backdrop="false" 
            style="background-color: rgba(0, 0, 0, 0.5); padding-top:5%;">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    ...
                    <!-- Modal body -->
                    <div class="modal-body d-flex flex-wrap justify-content-start" id="allLibPics{{$kategorit->id}}Tel">
                        @foreach(PicLibrary::all()->sortByDesc('created_at') as $pl)
                            <div style="width:33%;  border:1px solid lightgray;" class="mb-1 selectPic" data-dismiss="modal" onclick="selectPicForCatLibTel('{{$pl->picLPhoto}}','{{$pl->picLTitle}}','{{$kategorit->id}}')">
...
<script>
        function selectPicForCatLibTel(pic,title, catId){
            $('#selPic'+catId+'Phase01Tel').html("");
            $('#selPic'+catId+'Phase02Tel').css("display", "block");
            $('#selPic'+catId+'Phase02NameTel').html(pic);
            $('#selPic'+catId+'Phase02TitleTel').html("\" "+title+" \"");

            $('#photoFromID'+catId+'Tel').val('2');
            $('#photoID'+catId+'Tel').val(pic);
        }

        function resetAddCatModalPIcLib(catId){
            $("#addCatModalPhotoSel"+catId+"Tel").load(location.href+" #addCatModalPhotoSel"+catId+"Tel>*","");
        }
...
@foreach (Produktet::Where('toRes',Auth::user()->sFor)->get() as $pro)
    <div class="modal  fade " id="editProduktModal{{$pro->id}}Tel" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="false" style="background-color: rgba(0, 0, 0, 0.5); padding-top:5%;">
                ...
                <div class="modal-body" >
                    ...
                    <div class="row mt-1 mb-2" >
                        <div class="col-12 form-check" style="margin-left: 15px;">
                            ...
                            <label class="form-check-label pl-2" for="accessByClients{{$pro->id}}"><strong>Kunden können dieses Produkt bestellen</strong></label>
                    ...
                    <div class="row">
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <select name="kategoriaProdEdit" class="form-control" onchange="resetExtraTypesTel(this.value, '{{$theResId}}','{{$pro->id}}')">
                    ...
                    <div id="editProduktModalBodyExTyTel">
                        <?php
                            $step = 1;
                        ...
                        {{ Form::hidden('thisProdsExtrasIn',$thisProdsExtrasIn, ['class' => 'form-control', 'id' => 'thisProdsExtrasIn'.$pro->id.'Tel']) }}
                        {{ Form::hidden('thisProdsTypesIn',$thisProdsTypesIn, ['class' => 'form-control', 'id' => 'thisProdsTypesIn'.$pro->id.'Tel']) }}
                   
                        {{ Form::hidden('isAccountant','1', ['class' => 'form-control']) }}

                        <div id="BoxKatEdit{{$theResId}}P{{$pro->id}}Tel">
                            <div class="container">
                                <div class="row">
                                    <div class="col-6">
                                                ...
                                                <li class="list-group-item d-flex justify-content-between">
                                                    ...
                                                    <label style="width:19%;" class="text-right switch " >
                                                        @if(in_array($ekstras->id,$thisProdsExtras))
                                                            <input type="checkbox" class="success" id="activeExtraPro{{$ekstras->id}}P{{$pro->id}}Tel" checked 
                                                            onclick="removeThisExtraToProdTel('{{$ekstras->id}}','{{$pro->id}}','{{$kats->id}}','{{$theResId}}')">
                                                            <span class="slider round"></span>
                                                        @else
                                                            <input type="checkbox" class="success" id="activeExtraPro{{$ekstras->id}}P{{$pro->id}}Tel" 
                                                            onclick="addThisExtraToProdTel('{{$ekstras->id}}','{{$pro->id}}','{{$kats->id}}','{{$theResId}}')">
                                                            <span class="slider round"></span>
                                                        @endif
                                                    </label>
                                                </li>
                                                ...
                                                <li class="list-group-item d-flex justify-content-between">
                                                    ...
                                                    <label  style="width:19%;" class="text-right switch">
                                                        @if(in_array($proLl->id,$thisProdsTypes))
                                                            <input type="checkbox" class="success" id="activeTypePro{{$proLl->id}}P{{$pro->id}}Tel" checked
                                                            onclick="removeThisTypeToProdTel('{{$proLl->id}}','{{$pro->id}}','{{$kats->id}}','{{$theResId}}')">
                                                            <span class="slider round"></span>
                                                        @else
                                                            <input type="checkbox" class="success" id="activeTypePro{{$proLl->id}}P{{$pro->id}}Tel"
                                                            onclick="addThisTypeToProdTel('{{$proLl->id}}','{{$pro->id}}','{{$kats->id}}','{{$theResId}}')">
                                                            <span class="slider round"></span>
                                                        @endif
                                                    </label>
                                                </li>
...
<script>
    function addThisExtraToProdTel(exId , proId, catId, resId){
        var newthisProdsExtrasIn = $('#thisProdsExtrasIn'+proId+'Tel').val();
       
        if(newthisProdsExtrasIn == ''){ newthisProdsExtrasIn = exId; }
        else{ newthisProdsExtrasIn += '||'+exId; }
       
        $('#thisProdsExtrasIn'+proId+'Tel').val(newthisProdsExtrasIn);

        $('#activeExtraPro'+exId+'P'+proId+'Tel').attr('onclick','removeThisExtraToProdTel(\''+exId+'\',\''+proId+'\',\''+catId+'\',\''+resId+'\')');
    }

    function removeThisExtraToProdTel(exId , proId, catId, resId){
        var newthisProdsExtrasIn = '';
        var data = $('#thisProdsExtrasIn'+proId+'Tel').val().split('||');
        ...
        $('#activeExtraPro'+exId+'P'+proId+'Tel').attr('onclick','addThisExtraToProdTel(\''+exId+'\',\''+proId+'\',\''+catId+'\',\''+resId+'\')');
    }



    function addThisTypeToProdTel(tyId , proId, catId, resId){
        var newthisProdsTypesIn = $('#thisProdsTypesIn'+proId+'Tel').val();
       
        if(newthisProdsTypesIn == ''){ newthisProdsTypesIn = tyId; }
        else{ newthisProdsTypesIn += '||'+tyId; }
        $('#thisProdsTypesIn'+proId+'Tel').val(newthisProdsTypesIn);

        $('#activeExtraPro'+tyId+'P'+proId+'Tel').attr('onclick','removeThisTypeToProdTel(\''+tyId+'\',\''+proId+'\',\''+catId+'\',\''+resId+'\')');
    }

    function removeThisTypeToProdTel(tyId , proId, catId, resId){
        var newthisProdsTypeIn = '';
        var data = $('#thisProdsTypesIn'+proId+'Tel').val().split('||');
        $.each( data, function( index, value ) {
            if(value != tyId){
        ...
        $('#thisProdsTypesIn'+proId+'Tel').val(newthisProdsTypeIn);

        $('#activeExtraPro'+tyId+'P'+proId+'Tel').attr('onclick','addThisTypeToProdTel(\''+tyId+'\',\''+proId+'\',\''+catId+'\',\''+resId+'\')');
    }
    ...
    function resetExtraTypesTel(katId, resId, proId){
        $('#BoxKatEdit'+resId+'P'+proId+'Tel').html('<img style="width:40%; margin-left:30%;" src="storage/gifs/loading.gif" alt="">');
            ...
            success: function(res){
                $.each(res, function(index, value){
                    newExTyShow += '<li class="list-group-item d-flex justify-content-between">'+
                                        ...
                                        '<label style="width:19%;" class="text-right switch " >'+
                                            '<input type="checkbox" class="success" id="activeExtraPro'+value.id+'P'+proId+'Tel" onclick="addThisExtraToProdTel(\''+value.id+'\',\''+proId+'\',\''+katId+'\',\''+resId+'\')"><span class="slider round"></span>'+
                                        '</label>'+
                                    '</li>';
                });// end foreach extra
                    ...
                    success: function(res){
                        $.each(res, function(index, value){
                            newExTyShow += '<li class="list-group-item d-flex justify-content-between">'+
                                                ...
                                                '<label  style="width:19%;" class="text-right switch">'+
                                                    '<input type="checkbox" class="success" id="activeTypePro'+value.id+'P'+proId+'Tel" onclick="addThisTypeToProdTel(\''+value.id+'\',\''+proId+'\',\''+katId+'\',\''+resId+'\')">'+
                                                    '<span class="slider round"></span>'+
                                                '</label>'+
                                            '</li>';
                        });// end foreach tipe
                        newExTyShow +=  '</ul>'+
                            ...
                            '</div>'+  
                        '</div>';
                        $('#BoxKatEdit'+resId+'P'+proId+'Tel').html('');
                        $('#BoxKatEdit'+resId+'P'+proId+'Tel').append(newExTyShow);
                    }
                });
            }
        });
        ...
        $('#thisProdsTypesIn'+proId+'Tel').val('');
        $('#thisProdsExtrasIn'+proId+'Tel').val('');
    }
