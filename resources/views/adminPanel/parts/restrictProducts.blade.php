        ...
        <?php echo ' <div style="cursor: pointer; position:relative; object-fit: cover;" class="col12 p-1 text-center"
          onclick="showProKat('.$kat->id.')">'?>
            ...
            <ul class="ProdLists" id="prodsOf{{$kat->id}}" style="display:none;">
                @foreach(Produktet::where('toRes', Auth::user()->sFor)->where('kategoria', $kat->id)->get()->sortByDesc('visits') as $allPro)
                    <li id="prodListing{{$allPro->id}}">
                        <div class="d-flex">
                            ...
                            <span style="width:49%;" class=" col-6">
                                ...
                                @else
                                    <button onclick="setRestrict16('{{$allPro->id}}')" class="btn btn-outline-dark" style="width:32%;"> <strong>16</strong> </button>
                                @endif
                                @if($allPro->restrictPro == 18)
                                    <button class="btn btn-dark" style="width:32%;"> <strong>18</strong> </button>
                                @else
                                    <button onclick="setRestrict18('{{$allPro->id}}')" class="btn btn-outline-dark" style="width:32%;"> <strong>18</strong> </button>
                                @endif
                                @if($allPro->restrictPro == 0)
                                    <button class="btn btn-dark" style="width:32%;"> {{__('adminP.none')}} </button>
                                @else
                                    <button onclick="setRestrict0('{{$allPro->id}}')" class="btn btn-outline-dark" style="width:32%;"> {{__('adminP.none')}} </button>
                                @endif
               ...
               function searchProdsFuncREC() {
                ...
                function setRestrict16(proId){
                    $.ajax({
                        ...
                        success: () => {
                            $("#prodListing"+proId).load(location.href+" #prodListing"+proId+">*","");
                        },
                        error: (error) => {
                            console.log(error);
