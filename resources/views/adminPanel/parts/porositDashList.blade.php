...
<input type="hidden" value="{{$thisRestaurantId}}" id="theResId">
 ...
 <div class="col-12 pb-4" id="porositDash">
    ...
    @foreach(Orders::where([['Restaurant', Auth::user()->sFor],['created_at', '>=', $dateTimeNew]])->orderByDesc('created_at')->get() as $order)
        <?php
			...
			@if($agent->isTablet())  
                ...
                @foreach(Orders::where([['Restaurant', Auth::user()->sFor],['created_at', '>=', $dateTimeNew]])->orderByDesc('created_at')->get() as $order) 
                    ...
                    <tr class="openOrderRow tabletRows" id="orderListingShow{{$order->id}}">
                        ...
                        <td>
                        @if($order->statusi == 0)
                            <button class="btn btn-warning btn-block shadow-none" data-toggle="modal" data-target="#ChStat{{$order->id}}">{{__("adminP.waitingLine")}}<br>
                        ...
                        @elseif($order->statusi == 1)
                            <button class="btn btn-info btn-block shadow-none" data-toggle="modal" data-target="#ChStat{{$order->id}}">{{__("adminP.confirmed")}}<br>
                                @if($order->StatusBy!= 999999)
                                    @if(User::find($order->StatusBy) !=null)
                                        {{__('adminP.from')}} {{User::find($order->StatusBy)->name}}
                                    @endif
                        ...
                        @elseif($order->statusi == 2)
                            <button class="btn btn-danger btn-block shadow-none" data-toggle="modal" data-target="#ChStat{{$order->id}}" disabled>{{__("adminP.canceled")}}<br>
                                @if($order->StatusBy!= 999999)
                                    @if(User::find($order->StatusBy) != null)
                                        {{__('adminP.from')}} {{User::find($order->StatusBy)->name}}
                                    @endif
                        ...
                        @elseif($order->statusi == 3)
                            <button class="btn btn-success btn-block shadow-none" data-toggle="modal" data-target="#ChStat{{$order->id}}">{{__("adminP.finished")}}<br>
                                @if($order->StatusBy!= 999999)
                                    @if(User::find($order->StatusBy) !=null)
                                        {{__('adminP.from')}} {{User::find($order->StatusBy)->name}}
                                    @endif
...
<script>
    if ($(window).width() <= 580) {
        ...
        $('#desktopTableHeader').show();
        $('#topDesktopDate1').show();
        $('#topDesktopDate2').show();
        $('#timeDesktop').show();
