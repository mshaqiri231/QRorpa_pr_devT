...
style="background-color: rgba(0, 0, 0, 0.5); padding-top:1%;">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body p-1" id="groupMngGropL1ModBody">
                @if (cntGroupAdmWai::where('forUser',Auth::user()->id)->count() > 0)
                    <div class="d-flex flex-wrap justify-content-between">
                        <p style="color:rgb(39,190,175);"><strong>Kopieren Sie Ihre Kategoriegruppierung direkt auf Ihre Kellner</strong></p>
                        @foreach (User::where([['role',55],['sFor',Auth::user()->sFor]])->get() as $oneWai)
                            <button onclick="prepCopyGrToWaiter('{{$oneWai->id}}','{{$oneWai->name}}')" style="width: 100%; margin:0px;" class="btn btn-outline-info shadow-none text-center mb-1"
                            data-toggle="modal" data-target="#copyToWaiterModal">
                                <strong>{{$oneWai->name}}</strong>
                            </button>
                        @endforeach
                        <div class="alert alert-success text-center" id="copyGrToWaiterSuc01" style="display:none; width:100%;">
                            <strong>Gruppierungseinstellung erfolgreich übertragen</strong>
                        </div>
                        <hr style="width: 100%;">
                    </div>
                @endif
...
<div id="allGrModals">
    ...
    <!-- Copy to Waiter confirmation Modal -->
    <div class="modal" id="copyToWaiterModal" tabindex="-1" role="dialog" aria-labelledby="copyToWaiterModalLabel" data-backdrop="false" 
    style="background-color: rgba(0, 0, 0, 0.5); padding-top:1%;">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="copyToWaiterModalLabel"><strong>Kopieren Sie Ihre Kategoriegruppierung direkt auf Ihren Kellner namens <span id="cpyToWaName"></span></strong></h5>
                    <button type="button" class="close shadow-none" aria-label="Close" onclick="cancelCopyGrToWaiter()">
                        <span aria-hidden="true"><i class="far fa-times-circle"></i></span>
                    </button>
                </div>
                <div class="modal-body d-flex justify-content-between flex-wrap">
                    <input type="hidden" id="cpyToWaId" value="0">

                    <button class="btn btn-success" style="margin:0px; width:49%;" onclick="copyGrToWaiter()">Fortsetzen</button>
                    <button class="btn btn-dark" style="margin:0px; width:49%;" onclick="cancelCopyGrToWaiter()">Abbrechen</button>

                    <p class="mt-4" style="width:100%; color:red; font-size:0.75rem;">Wenn Sie fortfahren, wird jede von Ihnen erstellte Gruppierungseinstellung auch auf diesen Kellner kopiert, und er kann sie später ändern</p>
                </div> 
            </div>
        </div>
    </div>
...
<script>
    ...
    function showTime() {
        var today = new Date()
        $("#oraTel").html(pad(today.getHours())+':'+pad(today.getMinutes())+':'+pad(today.getSeconds()));
        $('#dataTel').html(pad(today.getDate())+'/'+pad((today.getMonth()+1)))
    }
    $( document ).ready(function() {
       setTimeTel = setInterval(showTime, 1000);
    });
        ...
        if(!$('#regGrL1InputName').val()){
            ...
            $.ajax({
				...
				success: () => {
                    ...
                    $("#groupMngGropL1ModBody").load(location.href+" #groupMngGropL1ModBody>*","");
    ...
    function setCatToGroupL2(grl1Id, grl2Id, katId){
        ...
        $.ajax({
	    	...
	    	success: () => {
                $("#newProductOrPage2ModalTelBody2").load(location.href+" #newProductOrPage2ModalTelBody2>*","");
    ...
    function prepCopyGrToWaiter(waId,waName){
        $('#cpyToWaId').val(waId);
        $('#cpyToWaName').html(waName);
    }
    function cancelCopyGrToWaiter(){
        $('#copyToWaiterModal').modal('toggle');
        $('#copyToWaiterModal').removeClass('show');
        $('body').attr('class','modal-open');
    }
    function copyGrToWaiter(){
        if($('#cpyToWaId').val() == 0){

        }else{
            $.ajax({
				url: '{{ route("adminGroup.copyGrToWaiters") }}',
				method: 'post',
				data: {
					waiId: $('#cpyToWaId').val(),
					admId: '{{Auth::user()->id}}',
				...
				success: () => {
                    $('#copyGrToWaiterSuc01').show(100).delay(4000).hide(100);
                    cancelCopyGrToWaiter();
				},
				error: (error) => { console.log(error); }
			});
        }
