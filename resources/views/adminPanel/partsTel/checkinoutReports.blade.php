...
<div class="pb-4">
    ...
    <div class="d-flex flex-wrap justify-content-between mb-3">
        <p style="font-weight: bold; font-size: 1.3rem; color:rgb(39,190,175); width:100%;">Check-in/out Berichte</p>
        <button class="ml-1 mr-1 btn btn-outline-dark shadow-none" style="width:100%;" data-toggle="modal" data-target="#waitersCheckin"><strong>Kellner checken ein/aus</strong></button>
    </div>
   
    <div class="p-1 d-flex flex-wrap justify-content-start" id="checkInOutRepDiv">
        @foreach (checkInOutReg::where([['userId',Auth::user()->id],['theStat',1]])->orderByDesc('updated_at')->get() as $chInOutOne)
            ...
            ?>
            <div style="width:100%; border:1px solid black;" class="d-flex flex-wrap mb-1" >
            ...
            <div class="modal-body">
                <div class="d-flex flex-wrap justify-content-start">
                    ...
                    @foreach (User::where([['role',55],['sFor',Auth::user()->sFor]])->get() as $waOne)
                        <div style="width:100%; border:1px solid black; border-radius:10px; margin-right:0.16%; margin-left:0.16%;" class="d-flex flex-wrap mb-1 clickable" onclick="showWaChinOut('{{Auth::user()->id}}','{{$waOne->id}}','{{$waOne->name}}')">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <table class="table table-bordered">
                    <tbody>
                        <tr>
                            <td style="font-size:1rem;"><strong>Bestellungen bestätigt</strong></td>
                            <td style="font-size:1rem;" class="text-center" id="orderClosedNr2"> --- </td>
                        </tr>
                        <tr>
                            <td style="font-size:1rem;"><strong>Bestellungen bestätigt</strong></td>
                            <td style="font-size:1rem;" class="text-center" id="orderClosedChf2"> --- </td>
                        </tr>
                        <tr>
                            <td style="font-size:1rem;"><strong>Geschenkkartenverkauf</strong></td>
                            <td style="font-size:1rem;" class="text-center" id="totalGCSalesCHF"> --- </td>
                        </tr>
                        <tr>
                            <td style="font-size:1rem;"><strong>Trinkgeld registriert</strong></td>
                            <td style="font-size:1rem;" class="text-center" id="bakshish12"> --- </td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    function showWaChinOut(admId,waId,waName){
        ...
        $.ajax({
			...
			success: (respo) => {
                ...
                if(admId != waId){
                    $('#checkInOutRepDiv').append('<button class="btn shadow-none" style="width:15%; font-size:0.9rem; padding:0px; margin-bottom:10px;" onclick="showWaChinOut(\''+admId+'\',\''+admId+'\',\'empty\')"><i class="fa-regular fa-lg fa-circle-left"></i></button>');
                    $('#checkInOutRepDiv').append('<p style="width:85%; font-weight:bold; font-size:1.1rem; margin-bottom:10px;" class="text-center">Instanzen für: '+waName+'</p>');
                }
                $.each(respo, function(index, value){
                    ...
                    addTo = '<div style="width:100%; border:1px solid black; " class="d-flex flex-wrap mb-1" >'+
                                '<p class="text-center mb-1" style="width:50%; font-size:1.2rem; font-weight:bold;">'+dtIn2D[2]+'.'+dtIn2D[1]+'.'+dtIn2D[0]+'</p>'+
                                '<p class="text-center mb-1" style="width:50%; font-size:1.2rem; font-weight:bold;">'+hrIn2D[0]+':'+hrIn2D[1]+'</p>'+
                                '<p class="text-center mb-1" style="width:100%; font-size:1.2rem; font-weight:bold;">bis</p>'+
                                '<p class="text-center mb-1" style="width:50%; font-size:1.2rem; font-weight:bold;">'+dtOut2D[2]+'.'+dtOut2D[1]+'.'+dtOut2D[0]+'</p>'+
