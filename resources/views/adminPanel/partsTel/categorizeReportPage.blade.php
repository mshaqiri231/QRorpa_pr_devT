...
<div class="p-1 pb-5">
    ...
    <div class="d-flex flex-wrap justify-content-between mt-1 mb-1">
        ...
        <h5 style="color:rgb(39,190,175); width:100%;">
            <strong> 
                <a href="{{route('dash.statistics')}}" class="pl-1 pr-2" style="width: 10%; color:rgb(39,190,175);"><strong><i class="fas fa-chevron-left"></i></strong></a> 
    ...
    <div class="p-1 d-flex flex-wrap justify-content-between" id="repCategories">
        @if ($pdfResPCats->count() > 0)
            ...
            <p style="width: 15%;" class="text-center">{{Produktet::where([['toRes',Auth::user()->sFor],['toReportCat',$oneRepCat->id]])->count()+Takeaway::where([['toRes',Auth::user()->sFor],['toReportCat',$oneRepCat->id]])->count()}}</p>
            ...
            <p style="width: 15%;" class="text-center">{{Produktet::where([['toRes',Auth::user()->sFor],['toReportCat',0]])->count()+Takeaway::where([['toRes',Auth::user()->sFor],['toReportCat',0]])->count()}}</p>
            <p style="width: 26%;"></p>
        @else
