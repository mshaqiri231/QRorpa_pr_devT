<?php
    ...
    $D2Li01Sum = (float)0; 
    $D3Li01Sum = (float)0;  
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng0']])->whereDate('created_at',$nowDate)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng0']])->whereDate('created_at',$nowDate)->get() as $oD0){
            if(Orders::find($oD0->actId) != NULL){ $D0Li01Sum = $D0Li01Sum + (float)Orders::find($oD0->actId)->shuma; }
        }
    }
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng0']])->whereDate('created_at',$Day_1)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng0']])->whereDate('created_at',$Day_1)->get() as $oD1){
            if(Orders::find($oD1->actId) != NULL){ $D1Li01Sum = $D1Li01Sum + (float)Orders::find($oD1->actId)->shuma; }
        }
    }
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng0']])->whereDate('created_at',$Day_2)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng0']])->whereDate('created_at',$Day_2)->get() as $oD2){
            if(Orders::find($oD2->actId) != NULL){ $D2Li01Sum = $D2Li01Sum + (float)Orders::find($oD2->actId)->shuma; }
        }
    }
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng0']])->whereDate('created_at',$Day_3)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng0']])->whereDate('created_at',$Day_3)->get() as $oD3){
    ...
    $D2Li02Sum = (float)0; 
    $D3Li02Sum = (float)0;  
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng1']])->whereDate('created_at',$nowDate)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng1']])->whereDate('created_at',$nowDate)->get() as $oD0){
            if(Orders::find($oD0->actId) != NULL){ $D0Li02Sum = $D0Li02Sum + (float)Orders::find($oD0->actId)->shuma; }
        }
    }
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng1']])->whereDate('created_at',$Day_1)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng1']])->whereDate('created_at',$Day_1)->get() as $oD1){
            if(Orders::find($oD1->actId) != NULL){ $D1Li02Sum = $D1Li02Sum + (float)Orders::find($oD1->actId)->shuma; }
        }
    }
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng1']])->whereDate('created_at',$Day_2)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng1']])->whereDate('created_at',$Day_2)->get() as $oD2){
            if(Orders::find($oD2->actId) != NULL){ $D2Li02Sum = $D2Li02Sum + (float)Orders::find($oD2->actId)->shuma; }
        }
    }
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng1']])->whereDate('created_at',$Day_3)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng1']])->whereDate('created_at',$Day_3)->get() as $oD3){
    ...
    $D2Li03Sum = (float)0; 
    $D3Li03Sum = (float)0;  
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng2']])->whereDate('created_at',$nowDate)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng2']])->whereDate('created_at',$nowDate)->get() as $oD0){
            if(Orders::find($oD0->actId) != NULL){ $D0Li03Sum = $D0Li03Sum + (float)Orders::find($oD0->actId)->shuma; }
        }
    }
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng2']])->whereDate('created_at',$Day_1)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng2']])->whereDate('created_at',$Day_1)->get() as $oD1){
            if(Orders::find($oD1->actId) != NULL){ $D1Li03Sum = $D1Li03Sum + (float)Orders::find($oD1->actId)->shuma; }
        }
    }
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng2']])->whereDate('created_at',$Day_2)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng2']])->whereDate('created_at',$Day_2)->get() as $oD2){
            if(Orders::find($oD2->actId) != NULL){ $D2Li03Sum = $D2Li03Sum + (float)Orders::find($oD2->actId)->shuma; }
        }
    }
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng2']])->whereDate('created_at',$Day_3)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng2']])->whereDate('created_at',$Day_3)->get() as $oD3){
    ...
    $D2Li04Sum = (float)0; 
    $D3Li04Sum = (float)0;  
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng3']])->whereDate('created_at',$nowDate)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng3']])->whereDate('created_at',$nowDate)->get() as $oD0){
            if(Orders::find($oD0->actId) != NULL){ $D0Li04Sum = $D0Li04Sum + (float)Orders::find($oD0->actId)->shuma; }
        }
    }
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng3']])->whereDate('created_at',$Day_1)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng3']])->whereDate('created_at',$Day_1)->get() as $oD1){
            if(Orders::find($oD1->actId) != NULL){ $D1Li04Sum = $D1Li04Sum + (float)Orders::find($oD1->actId)->shuma; }
        }
    }
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng3']])->whereDate('created_at',$Day_2)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng3']])->whereDate('created_at',$Day_2)->get() as $oD2){
            if(Orders::find($oD2->actId) != NULL){ $D2Li04Sum = $D2Li04Sum + (float)Orders::find($oD2->actId)->shuma; }
        }
    }
    if(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng3']])->whereDate('created_at',$Day_3)->count() > 0){
        foreach(waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng3']])->whereDate('created_at',$Day_3)->get() as $oD3){
...
<input type="hidden" id="D0L1Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng0']])->whereDate('created_at',$nowDate)->sum('sasia')}}">
<input type="hidden" id="D1L1Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng0']])->whereDate('created_at',$Day_1)->sum('sasia')}}">
<input type="hidden" id="D2L1Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng0']])->whereDate('created_at',$Day_2)->sum('sasia')}}">
<input type="hidden" id="D3L1Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng0']])->whereDate('created_at',$Day_3)->sum('sasia')}}">

<input type="hidden" id="D0L2Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng1']])->whereDate('created_at',$nowDate)->sum('sasia')}}">
<input type="hidden" id="D1L2Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng1']])->whereDate('created_at',$Day_1)->sum('sasia')}}">
<input type="hidden" id="D2L2Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng1']])->whereDate('created_at',$Day_2)->sum('sasia')}}">
<input type="hidden" id="D3L2Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng1']])->whereDate('created_at',$Day_3)->sum('sasia')}}">

<input type="hidden" id="D0L3Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng2']])->whereDate('created_at',$nowDate)->sum('sasia')}}">
<input type="hidden" id="D1L3Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng2']])->whereDate('created_at',$Day_1)->sum('sasia')}}">
<input type="hidden" id="D2L3Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng2']])->whereDate('created_at',$Day_2)->sum('sasia')}}">
<input type="hidden" id="D3L3Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng2']])->whereDate('created_at',$Day_3)->sum('sasia')}}">

<input type="hidden" id="D0L4Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng3']])->whereDate('created_at',$nowDate)->sum('sasia')}}">
<input type="hidden" id="D1L4Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng3']])->whereDate('created_at',$Day_1)->sum('sasia')}}">
<input type="hidden" id="D2L4Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng3']])->whereDate('created_at',$Day_2)->sum('sasia')}}">
<input type="hidden" id="D3L4Nr" value="{{waiterActivityLog::where([['waiterId',$theW->id],['actType','takeawayOrStatChng3']])->whereDate('created_at',$Day_3)->sum('sasia')}}">
