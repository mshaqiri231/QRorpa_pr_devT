...
<div class="container-fluid mb-4 " >
    <div class="row mt-4">
        ...
        <div class="col-8 text-center">
            <p style="font-size:45px;" class="color-qrorpa">"{{Restorant::find(Auth::user()->sFor)->emri}}"  /  {{__('adminP.categories')}}</p>
        </div>
         <div class="col-2"></div>
    </div>
</div>
    ...
    <div class="container-fluid p-2 kategoriteAll mb-3" id="KategoriOne{{$theResId}}">
        <div class="row">
            <div class="col-12">
                @if(session('error'))
        ...
        <div class="row">
            <div class="col-12">
                <button type="button" class="btn btn-outline-dark btn-block" data-toggle="modal" data-target="#addCatModal">
                    ...
                    <strong> {{__('adminP.addCategory')}} </strong>
                </button>
            </div>
        </div>    
    </div>
    ...
    <div class="pr-5 pl-5 pb-5">
        @foreach($kat as $kategorit)
            <div class="d-flex justify-content-between flex-wrap mb-1 p-2 bg-light" >
                ...
                <div style="width:20%;" class="text-center">
                    ...
                    <button class="btn btn-block btn-block mb-3" style="color:blue; font-size:19px;" data-toggle="modal" data-target="#editKat{{$kategorit->id}}"><strong>{{__('adminP.edit')}}</strong></button>
                            
                        {{Form::open(['action' => ['KategoriController@destroy', $kategorit->id ], 'method' => 'POST', 'onsubmit' => 'return confirm("__('adminP.areYouSure')")'])}}

                        {{Form::hidden('_method', 'DELETE')}}
                        {{Form::submit(__('adminP.delete'), ['class' => 'btn btn-block', 'style' => 'font-weight:bold; color:red; font-size:19px;' ])}}
                ...
                <div style="border:1px solid rgb(39,190,175,0.6); width:100%; border-radius:10px;" class="p-1 mt-3 d-flex flex-wrap justify-content-start">
                    <p style="text-align: center; width:100%;  margin-top:-20px; font-size:20px; z-index:99;"><strong>{{__('adminP.type')}}</strong></p>
                ...
                <div style="width:100%; border:1px solid rgb(39,190,175,0.6); border-radius:10px;" class="p-1 mt-3 d-flex flex-wrap justify-content-start">
                    <p class="text-center" style="width:100%; margin-top:-20px; font-size:20px;"><strong>{{__('adminP.extra')}}</strong></p>
                    @foreach(ekstra::where([['toRes',$theResId],['toCat',$kategorit->id]])->get() as $ext)
                        <span style="width:14.25%; border:1px solid rgb(39,190,175,0.4);" class="p-1 mb-1 text-center">
                            {{$ext->emri}} <span style="opacity:0.7;">({{$ext->qmimi}} {{__('adminP.currencyShow')}})</span>
                         </span>
                    @endforeach
                </div>
                @endif
    ...
    style="background-color: rgba(0, 0, 0, 0.5); margin-top:5%;">
            <div class="modal-dialog modal-lg">
                ...
                <div class="modal-header">
                    <h4 class="modal-title" style="color:black;">{{__('adminP.addCategoryToRestaurant')}}</h4>
                    <button type="button" class="btn btn-danger" data-dismiss="modal">{{__('adminP.conclude')}}</button>
...
style="background-color: rgba(0, 0, 0, 0.5); margin-top:5%;">
    <div class="modal-dialog modal-lg">
        ...
        <!-- Modal Header -->
        <div class="modal-header">
            <h4 class="modal-title">{{__('adminP.youEdit')}} "{{$kategorit->emri}}"</h4>
            <button type="button" class="btn btn-danger" data-dismiss="modal">{{__('adminP.conclude')}}</button>
            <!-- <button type="button" class="close" data-dismiss="modal">&times;</button> -->
        </div>
