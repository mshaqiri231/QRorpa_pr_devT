<?php
    ...
    if(taDeForCookOr::where([['toRes',Auth::User()->sFor],['serviceType','2']])->count() > 0 ){
        ...
        if(cooksProductSelection::where([['workerId',Auth::User()->id],['contentType','Category']])->count() > 0){
            $toFilterByPTE = array();
            foreach(taDeForCookOr::where([['toRes',Auth::User()->sFor],['serviceType','2']])->get()->sortByDesc('created_at') as $onePCatFilter){
            ...
            $toFilterByPTE = taDeForCookOr::where([['toRes',Auth::User()->sFor],['serviceType','2']])->get()->sortByDesc('created_at');
        }
...
@else
    ...
    <script>
        ...
        function addOneDone(deInstanceId,prodId){
            $('#addOneDoneBtn'+deInstanceId).html('<img style="width:23px; height:23px;" class="gifImg" src="storage/gifs/loading2.gif" alt=""> ')
            $('#addOneDoneBtn'+deInstanceId).prop('disabled',true);
            $.ajax({
                url: '{{ route("cookPnl.cookPanelAddOneDoneOrderProdD") }}',
                method: 'post',
                data: {
                    deInstId: deInstanceId,
                    _token: '{{csrf_token()}}'
                },
                success: () => {
        ...
        function removeOneDone(deInstanceId,prodId){
            $('#removeOneDoneBtn'+deInstanceId).html('<img style="width:23px; height:23px;" class="gifImg" src="storage/gifs/loading2.gif" alt=""> ')
            $('#removeOneDoneBtn'+deInstanceId).prop('disabled',true);
            $.ajax({
                url: '{{ route("cookPnl.cookPanelRemoveOneDoneOrderProdD") }}',
                method: 'post',
                data: {
                    deInstId: deInstanceId,
                    _token: '{{csrf_token()}}'
        ...
        function orderIsFinished(deInstanceId,prodId){
            $('#orderIsFinishedBtn'+deInstanceId).html('<img style="width:23px; height:23px;" class="gifImg" src="storage/gifs/loading2.gif" alt=""> ')
            $('#orderIsFinishedBtn'+deInstanceId).prop('disabled',true);
            $.ajax({
                url: '{{ route("cookPnl.cookPanelOrderProdFinishedD") }}',
                method: 'post',
                data: {
                    deInstId: deInstanceId,
                    _token: '{{csrf_token()}}'
