...
use App\resPlates;
    ...
    $allOrders = array();
    foreach(taDeForCookOr::where([['toRes',Auth::User()->sFor],['serviceType','1']])->orderByDesc('updated_at')->get() as $taOr){
...
@if(count($allOrders) > 0 )
    ...
    <div style="background-color: rgb(39,190,175); min-height: 50cm;" class="pr-1 pl-1 pt-1" id="cookPanelAll">
        <p class="text-center" style="width:100%; color: white; font-size:2rem;"><strong>Takeaway ( Bereit )</strong></p>
        ...
        <div class="swiper-container p-0" style="background-color:rgb(39,190,175) ;">
            <div class="swiper-wrapper p-2" style="background-color:rgb(39,190,175) ;">
                @foreach($allOrders as $orOne)
                    <?php
                        $tableShow = False;
                        foreach(taDeForCookOr::where([['toRes',Auth::User()->sFor],['serviceType','1'],['orderId',$orOne]])->orderByDesc('updated_at')->get() as $checkTToShowT){
                    ...
                    ?>
                    @if($tableShow)
                    <div class="swiper-slide d-flex flex-wrap" style="background-color:transparent; border-radius:6px; width:100%; flex-direction: row" id="orderColumnCookTO{{$orOne}}">
                        <?php
                        ...
                        <?php
                            $allPlates = array();
                            foreach(taDeForCookOr::where([['toRes',Auth::User()->sFor],['serviceType','1'],['orderId',$orOne]])->orderByDesc('updated_at')->get() as $taOr2){
                            ...
                            <div class="text-center pt-2 pb-2 mb-2 allPlate{{$orOne}}" style="width:100%; border-radius:6px; background-color:rgba(191,191,191,255); margin-bottom:0px; " 
                                ...
                                @endif
                                    ...
                                    <div style="width: 100%;" id="ordersListT{{$orOne}}P{{$plateOne}}">
                                        @foreach(taDeForCookOr::where([['toRes',Auth::User()->sFor],['serviceType','1'],['orderId',$orOne]])->orderByDesc('updated_at')->get() as $taOr3)
                                            @if($taOr3->prodSasia == $taOr3->prodSasiaDone)
                                                ...
                                                @if($tc2 != NULL && $tc2->forPlate == $plateOne)
                                                    ...
                                                    @if ($has2nAccess)
                                                        ...
                                                        @endif
                                                            <?php
                                                                $orUpTi = explode(':',explode(' ',$taOr3->updated_at)[1]);
                                                            ?>
                                                            ...
                                                            @if($taOr3->prodType != 'empty')
                                                            <h5 style="width:100%"><strong>Typ: {{explode('||',$taOr3->prodType)[0]}}</strong></h5>
                                                            @endif
        ...
        $.ajax({
            ...
            success: (respo) => {
                ...
                if(allSas == doneSas){
                    ...
                    if(notDone == 0){
                        $('#orderColumnCookTO'+tabNr).remove();
                    }else{
